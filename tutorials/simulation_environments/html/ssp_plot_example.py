...
import numpy as np
from ssp_utils import SpatialHeatmap, make_good_unitary, get_heatmap_vectors, encode_point
from shapes import image, canvas
import os
...
class Environment(object):
    ...
    def __init__(self, vocab_vectors, x_axis_sp, y_axis_sp, n_items=5):
        ...
        self.images = [
            'images/icons8-badger-96.png',
            'images/icons8-bear-96.png',
            'images/icons8-fox-96.png',
            'images/icons8-pug-96.png',
            'images/icons8-zebra-96.png'
        ]
    ...
    def __call__(self, t, x):
        ...
        for i in range(self.n_items):
            ...
            shape_list.append(
                image(height=1, width=1, x=x_pos+limit-.5, y=y_pos+limit-.5, img=self.images[identity])
            )

            output += encode_point(x_pos, y_pos, self.x_axis_sp, self.y_axis_sp) * spa.SemanticPointer(data=self.vocab_vectors[identity, :])

        self._nengo_html_ = canvas(shape_list, width=limit*2, height=limit*2)
...
with model:
    ...
    nengo.Connection(env, memory, synapse=None)

    # item selector goes to the first index
    nengo.Connection(item_selector, heatmap_node[0], synapse=None)

    # the memory itself goes to all other indices
    nengo.Connection(memory, heatmap_node[1:], synapse=None)