...
def kinect_vision(cid, handle, height=64, width=64):
	err, res, image = vrep.simxGetVisionSensorImage(cid, handle, 1, vrep_mode)
	if err == 1:
		return np.zeros(height * width)
	#ret,res,img=vrep.simxGetVisionSensorImage(cid,handle,0,vrep.simx_opmode_str)
	#ret,res,img=vrep.simxGetVisionSensorImage(cid, handle,0,vrep.simx_opmode_buffer)
	...
	#return np.array(image[i][j][0]).ravel()
	return image.ravel(order='A')
